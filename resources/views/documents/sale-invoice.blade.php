...
<x-document.layout
    :title="'Sales Invoice'"
    :subtitle="$sale->return_processed ? '(Return Processed)' : null"
...
>
    ...
    <x-document.parties
        :to="[
            ...
            'lines' => $isWalkIn ? ['No registered customer'] : [
                $customer?->phone,
                $customer?->email,
                $customer?->address,
        ...
        :extra="!$isWalkIn && $customer?->current_balance > 0 ? [
            ...
            'lines' => [
                ...
                $customer->credit_limit > 0 ? 'Credit Limit: ৳' . number_format($customer->credit_limit, 2) : 'Credit Limit: Unlimited',
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @if($item->serial_number)
                            <br><span class="mono" style="font-size:7.5pt;color:#1e40af;">IMEI: {{ $item->serial_number }}</span>
                        @endif
    ...
    @if($sale->payments->isNotEmpty())
        ...
        <div class="doc-two-col">
                    ...
                    <div class="doc-kv-row">
                        <span class="doc-kv-label">
                            {{ $pmt->paymentAccount?->name
                                ?? $pmt->financePartner?->name
                                ?? ucfirst(str_replace('_', ' ', $pmt->payment_type)) }}
            ...
            @if(!$isWalkIn && ($bakiTotal > 0 || $sale->due_collection_amount > 0))
                <div>
                    <div class="doc-kv-row">
                        <span class="doc-kv-label" style="color:#dc2626;">Credit / Baki this sale</span>
                    ...
                    @if($sale->due_collection_amount > 0)
                        <div class="doc-kv-row">
                            ...
                            <span class="doc-kv-value" style="color:#16a34a;">৳{{ number_format($sale->due_collection_amount, 2) }}</span>
                        </div>
                    @endif
                    <div class="doc-kv-row" style="border-top:1pt solid #1e3a5f;margin-top:1mm;padding-top:1mm;">
                        ...
                        <span class="doc-kv-value" style="color:#dc2626;">৳{{ number_format($customer?->current_balance ?? 0, 2) }}</span>
                    </div>
                </div>
            @endif
        </div>
    ...
    @if($sale->financePartnerReceivable)
        ...
        <div class="doc-two-col">
            <div>
                <div class="doc-kv-row"><span class="doc-kv-label">Partner</span><span class="doc-kv-value">{{ $fpr->financePartner?->name }}</span></div>
                <div class="doc-kv-row"><span class="doc-kv-label">EMI Receivable</span><span class="doc-kv-value">৳{{ number_format($fpr->total_amount, 2) }}</span></div>
                <div class="doc-kv-row"><span class="doc-kv-label">Status</span><span class="doc-kv-value">{{ $fpr->status->label() }}</span></div>
            </div>
        </div>
    @endif
    ...
    @if($bakiTotal <= 0 && $sale->status->value === 'confirmed')
        <div class="doc-stamp-container"><div class="doc-stamp doc-stamp-paid">PAID</div></div>
    @elseif($bakiTotal > 0)
        <div class="doc-stamp-container"><div class="doc-stamp doc-stamp-partial">PARTIAL</div></div>
    @endif
